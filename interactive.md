...
- [ ] **8.1** **Quiz model & storage:** Add a `learning_quizzes` collection to store quiz definitions and a `learning_quiz_results` collection for per-user attempts.
...
- [ ] **8.2** **Quiz generation by Michael:** Add `POST /api/learning/quizzes/generate` that asks Michael to create a 3–5 question quiz for a given `targetId` (lecture/practice). Persist the quiz as `createdBy: 'michael'`.
- [ ] **8.3** **Quiz retrieval & caching:** Add `GET /api/learning/quizzes?targetType=&targetId=` to fetch the latest quiz; if none exists, show “צור חידון עם מייקל” and generate on demand.
- [ ] **8.4** **UI embed per lecture/practice:** In the Interactive Learning page, add a “מיקרו-חידון” panel below the PDF viewer (or in a tab) that:
...
- [ ] **8.5** **SQL runner integration:** For `type: 'sql'` questions, reuse the existing infrastructure at `/homework/runner/[id]`:
...
- [ ] **8.6** **Result capture API:** Add `POST /api/learning/quizzes/attempts` to persist per-user attempts (`answers[]`, `score`, `feedbackByQuestion[]`), and return attempt metadata for UI use.
- [ ] **8.7** **Accessibility & i18n:** Ensure RTL layout, keyboard navigation, and `aria-live` feedback for correct/incorrect states; add localized labels for quiz actions.
...
- SQL validation uses the existing `/homework/runner/[id]` execution pipeline to keep grading consistent with current homework tooling.
